...
Goal_Think::Goal_Think(Raven_Bot* pBot):Goal_Composite<Raven_Bot>(pBot, goal_think)
{
  ...
  double FleeBias = RandInRange(LowRangeOfBias, HighRangeOfBias);
...
void Goal_Think::AddGoal_FleeCuzOfStress()
{
    if (notPresent(goal_flee_stress))
    {
        RemoveAllSubgoals();
        AddSubgoal(new FleeCuzOfStress(m_pOwner));
    }
}
